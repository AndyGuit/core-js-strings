...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str[i])) numOfVowels += 1;
  }
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (!refusedChars.includes(str[i])) {
...
function encodeToRot13(str) {
  ...
  const codedChars = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';

  let codedStr = '';

  for (let i = 0; i < str.length; i += 1) {
    const idx = originalChars.indexOf(str[i]);

    if (idx < 0) {
      codedStr += str[i];
    } else {
      codedStr += codedChars[idx];
    }
  }
